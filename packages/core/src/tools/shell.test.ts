...
describe('ShellTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should wrap command on linux and parse pgrep output', async () => {
      ...
      const wrappedCommand = `{ my-command & }; __code=$?; if command -v pgrep >/dev/null 2>&1; then pgrep -g 0 >${tmpFile} 2>&1; fi; exit $__code;`;
    ...
    it('should use the provided absolute directory as cwd', async () => {
      ...
      const tmpFile = path.join(os.tmpdir(), 'shell_pgrep_abcdef.tmp');
      const wrappedCommand = `{ ls; }; __code=$?; if command -v pgrep >/dev/null 2>&1; then pgrep -g 0 >${tmpFile} 2>&1; fi; exit $__code;`;
      expect(mockShellExecutionService).toHaveBeenCalledWith(
        wrappedCommand,
        subdir,
        expect.any(Function),

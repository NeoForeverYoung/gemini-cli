...
## 八、可观测与价值评估：用数据证明 Agent 的价值

在企业环境中，要判断某个 Agent 是否值得持续投入，核心是建立一套完整的可观测与评估体系，用数据回答“有没有人用、用得怎么样、值不值”。

### 1. 使用情况：这个 Agent 到底有没人用

- 用户侧指标
  - 每个 Agent 的 DAU / WAU / MAU（按开发者数）
  - 不同团队 / 项目的覆盖率（至少使用过一次的人数占比）
- 调用侧指标
  - 每个 Agent 每日 / 每周调用次数
  - 会话数、平均会话轮数
  - 调用入口分布：IDE 插件 / CLI / CI 流水线等
- 场景分布
  - 调用发生在哪些仓库 / 目录（核心服务 vs 边缘项目）
  - 主要任务类型：错误修复、重构、查文档、生成测试等

### 2. 效率收益：是否真正帮团队节省了时间

- 行为转化指标
  - Agent 生成建议的采纳率（最终被合入代码库的比例）
  - 半自动 vs 全自动：多少修改是“人审后小改接受”，多少可以直接接受
- 效率类指标（可以用估算模型，而非绝对精确）
  - 典型任务的“人工基线时间” vs “使用 Agent 的实际耗时”
  - 人均每周使用 Agent 完成的典型任务数量
- 流程级指标
  - CI 失败到修复的平均时长（MTTR）变化
  - 对同类问题的重复修复速度是否明显提升

### 3. 质量与风险：有没有把坑越挖越大

- 代码质量
  - Agent 产出修改在 CI 中的通过率（lint / test / typecheck）
  - 与 Agent 相关的变更被 revert 的比例
  - 代码评审中针对 Agent 生成部分的修改意见数量（可通过标记或关键字简单统计）
- 线上风险
  - 与 Agent 相关的线上故障 / 事故数（通过变更记录与故障单关联）
  - 事故等级分布：是小缺陷还是高优故障

### 4. 数据采集与埋点设计（工程视角）

- 统一事件模型
  - 每次 Agent 调用或关键动作写一条事件，包含：
    - agent_id / agent_version
    - user_id / team / project_id / repo
    - 动作类型（发起调用、生成建议、被采纳、被修改、被回滚、导致 CI 失败 / 成功等）
    - 时间戳、耗时、Token in/out
    - 简单的结果标签（成功 / 失败 / 被用户拒绝）
- 日志管道
  - 将事件写入统一日志或埋点系统（如 Kafka + OLAP / 时序库 / ELK）
  - 保留足够长的历史，支持趋势分析和纵向对比

### 5. Dashboard 与决策闭环（管理视角）

- Agent 级健康看板
  - 每个 Agent 的使用量、活跃用户数、调用趋势
  - 建议采纳率、CI 通过率、revert 率
  - 估算节省人时 vs Token 成本（粗粒度 ROI）
- 团队 / 项目视图
  - 各团队对 Agent 的采纳率和使用深度
  - 哪些团队使用积极、哪些团队几乎不用
- 生命周期管理
  - 基于指标决策：
    - 哪些 Agent 值得继续投入优化；
...
通过这套可观测与评估体系，可以把“某个开发做了一个 Agent，到底有没有人用、有没有创造价值”量化出来，支撑后续的资源投入和产品演进决策。
